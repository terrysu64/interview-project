...
export async function getQuote(
  ...
  const routerContractAddress = "0xf164fC0Ec4E93095b804a4795bBe1e041497b92a";
  ...
  const uniswapContract = new Contract(routerContractAddress, routerABI, provider);
  
  fromToken.address = fromToken==ETH ? WETH.address : fromToken.address;
  toToken.address = toToken==ETH ? WETH.address : toToken.address;
  const amountsOut = await uniswapContract.getAmountsOut(fromAmount, [fromToken.address, toToken.address]);
  ...
  const currentAmountsOut = await uniswapContract.getAmountsOut(fromAmount, [fromToken.address, toToken.address]);
  const currSwapBalance = amountsOut[1];

  const slippagePercent = (currSwapBalance - swapBalance) / currSwapBalance;
